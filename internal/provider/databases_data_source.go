...
func (d *databasesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	for _, item := range res200.Data {
		...
		out := databaseDataSourceModel{
			...
			AllowDataBranching:                types.BoolValue(item.AllowDataBranching),
			AtBackupRestoreBranchesLimit:      types.BoolValue(item.AtBackupRestoreBranchesLimit),
			AtDevelopmentBranchLimit:          types.BoolValue(item.AtDevelopmentBranchLimit),
			AutomaticMigrations:               types.BoolValue(item.AutomaticMigrations),
			BranchesCount:                     types.Float64Value(item.BranchesCount),
			BranchesUrl:                       types.StringValue(item.BranchesUrl),
			CreatedAt:                         types.StringValue(item.CreatedAt),
			DefaultBranch:                     types.StringValue(item.DefaultBranch),
			DefaultBranchReadOnlyRegionsCount: types.Float64Value(item.DefaultBranchReadOnlyRegionsCount),
			DefaultBranchShardCount:           types.Float64Value(item.DefaultBranchShardCount),
			DefaultBranchTableCount:           types.Float64Value(item.DefaultBranchTableCount),
			DevelopmentBranchesCount:          types.Float64Value(item.DevelopmentBranchesCount),
			HtmlUrl:                           types.StringValue(item.HtmlUrl),
			InsightsRawQueries:                types.BoolValue(item.InsightsRawQueries),
			IssuesCount:                       types.Float64Value(item.IssuesCount),
			MigrationFramework:                types.StringPointerValue(item.MigrationFramework),
			MigrationTableName:                types.StringPointerValue(item.MigrationTableName),
			MultipleAdminsRequiredForDeletion: types.BoolValue(item.MultipleAdminsRequiredForDeletion),
			Notes:                             types.StringPointerValue(item.Notes),
			Plan:                              types.StringValue(item.Plan),
			ProductionBranchWebConsole:        types.BoolValue(item.ProductionBranchWebConsole),
			ProductionBranchesCount:           types.Float64Value(item.ProductionBranchesCount),
			Ready:                             types.BoolValue(item.Ready),
			RequireApprovalForDeploy:          types.BoolValue(item.RequireApprovalForDeploy),
			RestrictBranchRegion:              types.BoolValue(item.RestrictBranchRegion),
			SchemaLastUpdatedAt:               types.StringPointerValue(item.SchemaLastUpdatedAt),
			Sharded:                           types.BoolValue(item.Sharded),
