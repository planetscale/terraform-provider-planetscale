...
type PasswordWithPlaintext struct {
	...
	CreatedAt      string         `json:"created_at" tfsdk:"created_at"`
	DatabaseBranch PasswordBranch `json:"database_branch" tfsdk:"database_branch"`
...
type CreateBackupRes201 struct {
	Backup
}
...
func (cl *Client) CreateBackup(ctx context.Context, organization string, database string, branch string, req CreateBackupReq) (res201 *CreateBackupRes201, err error) {
	u := cl.baseURL.ResolveReference(&url.URL{Path: "organizations/" + organization + "/databases/" + database + "/branches/" + branch + "/backups"})
	body := bytes.NewBuffer(nil)
	if err = json.NewEncoder(body).Encode(req); err != nil {
		return res201, err
