...
type ListClusterSizeSkusForOrganizationRes_DataItem struct {
	...
	Name        string  `json:"name" tfsdk:"name"`
	Provider    string  `json:"provider" tfsdk:"provider"`
	Ram         float64 `json:"ram" tfsdk:"ram"`
	SortOrder   float64 `json:"sort_order" tfsdk:"sort_order"`
	Storage     float64 `json:"storage" tfsdk:"storage"`
}
type ListClusterSizeSkusForOrganizationRes struct {
	Data []ListClusterSizeSkusForOrganizationRes_DataItem `json:"data" tfsdk:"data"`
}
type ListClusterSizeSkusForOrganizationRes401 struct {
	*ErrorResponse
}
type ListClusterSizeSkusForOrganizationRes403 struct {
	*ErrorResponse
}
type ListClusterSizeSkusForOrganizationRes404 struct {
	*ErrorResponse
}
type ListClusterSizeSkusForOrganizationRes500 struct {
	*ErrorResponse
}

func (cl *Client) ListClusterSizeSkusForOrganization(ctx context.Context, name string) (res200 *ListClusterSizeSkusForOrganizationRes, err error) {
	u := cl.baseURL.ResolveReference(&url.URL{Path: "organizations/" + name + "/cluster-size-skus"})
	r, err := http.NewRequestWithContext(ctx, "GET", u.String(), nil)
	if err != nil {
		return res200, err
	}
	...
	switch res.StatusCode {
	case 200:
		res200 = new(ListClusterSizeSkusForOrganizationRes)
		err = json.NewDecoder(res.Body).Decode(&res200)
	case 401:
		res401 := new(ListClusterSizeSkusForOrganizationRes401)
		err = json.NewDecoder(res.Body).Decode(&res401)
		if err == nil {
			err = res401
		}
	case 403:
		res403 := new(ListClusterSizeSkusForOrganizationRes403)
		err = json.NewDecoder(res.Body).Decode(&res403)
		if err == nil {
			err = res403
		}
	case 404:
		res404 := new(ListClusterSizeSkusForOrganizationRes404)
		err = json.NewDecoder(res.Body).Decode(&res404)
		if err == nil {
			err = res404
		}
	case 500:
		res500 := new(ListClusterSizeSkusForOrganizationRes500)
		err = json.NewDecoder(res.Body).Decode(&res500)
		if err == nil {
			err = res500
		}
